...
module.exports = {
    ...
    customer_signup: async(req,res,next)=>{
        try{
            ...
            const foundUser = await User.findOne({email:req.body.email,role:"customer"});
            if (foundUser) {
            ...
            //send token
            res.status(201).json({message:"customer registered",token:token});
    ...
    customer_signin: async(req,res,next)=>{
        try{
