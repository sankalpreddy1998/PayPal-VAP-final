...
module.exports = {
    ...
    shipper_signup: async(req,res,next)=>{
        try{

            //check if user exist with an admin role
            const foundUser = await User.findOne({email:req.body.email,role:"shipper"});
            if (foundUser) {
            ...
            //send token
            res.status(201).json({message:"shipper registered",token:token});
    ...
    shipper_signin: async(req,res,next)=>{
        try{
